...
and right(Img_Name,2) <> '_t' and  not exists (select 1 from bl.bl_image_data b where a.img_name=b.img_name and b.attribute2='SVG')
union all
...
and right(Img_Name,2)= '_t' and not exists (select 1 from bl.bl_image_data b where a.img_name=b.img_name and b.attribute2='SVG_THUMB')
union all 
...
AND IMG_HASH IS NOT NULL
AND IMG_CALLOUT_HASH IS NULL
and right(Img_Name,2)= '_t' and img_name in (select concat(illust_no,'_t') as img_name from bl.bl_nna_illust_master)
union all 
