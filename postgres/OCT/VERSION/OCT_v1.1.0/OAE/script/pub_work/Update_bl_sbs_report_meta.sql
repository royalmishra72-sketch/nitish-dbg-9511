---- FDNGPUB-1647 - update oct_ddl_export_report report query to allow fetching the grants of the self schema
update bl.bl_sbs_report_meta
set row_last_update_ts = now(),row_last_update_user_id =current_user ,
report_query = '
/* 25th Feb 2025 - SonuA - Updated to allow fetching self schema grants*/
select schema_name,object_type,replace(object_name,''"'','''') as object_name,definition 
from 
(
...
SELECT acl_base.object_schema,
    ...
    --WHERE acl_base.grantor_oid <> acl_base.grantee_oid /* commented to allow fetching self schema grants*/
	) q
...
group by object_schema,object_type,object_name'
where report_name = 'oct_ddl_export_report';