...
declare
    ...
    Exported on:   2026-08-05 07:00:26-04
    From Database: nis_pub_dev_1_db

    With parameters
      p_like_filter: pub_work.image_data_w_v
...
pub_work.image_data_w_v';
-- Now compile the views that were exported
v_cnt := 0;


begin 
  v_tmp := af_repo.fk_compile_options( p_context_tx =>'pub_work.image_data_w_v',p_user_id=>'af_repo');
  v_ret := v_ret||chr(10)||'Compiled view: pub_work.image_data_w_v';
  v_cnt := v_cnt + 1;              
exception
  when others then 
    get stacked diagnostics
     ...
     v_ret := v_ret||chr(10)||'View context <pub_work.image_data_w_v> had compile failure: ' ||v_msg;
     v_err_cnt := v_err_cnt + 1;
     v_err_msg := v_err_msg||',pub_work.image_data_w_v';
end;
  raise info '%', v_ret;
